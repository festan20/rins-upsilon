"""Face detector node (Task 2).

Detects face posters using YOLOv8 person detection on OAK-D RGB + the organized
point cloud, transforms to the map frame and deduplicates.

Published topics
----------------
/detected_faces_task2     (geometry_msgs/PointStamped)  — one per detection;
                          frame_id encodes track: "map/<id>/<count>"
/face_markers_task2       (visualization_msgs/MarkerArray) — RViz visualisation
/face_detector_task2/debug (sensor_msgs/Image) — annotated BGR frame
"""
...
class FaceDetectorTask2Node(Node):
    ...
    def __init__(self):
        super().__init__('face_detector_task2')
        ...
        self.get_logger().info('Face detector (Task 2) ready.')
    ...
    def _publish_markers(self) -> None:
        ...
        for track in self.tracker.tracks:
            ...
            # Text label with count (black for white-map readability)
            t = Marker()
            t.header.frame_id = 'map'
            t.header.stamp = now
            t.ns = 'face_labels_task2'
            t.id = track['id']
...
def main():
    print('Face detection node (Task 2) starting.')
    rclpy.init(args=None)
    node = FaceDetectorTask2Node()
    try:
        rclpy.spin(node)
